...
with tab2:
    ...
    אם הרבה אנשים קונים מכם לימונדה — הדוכן שלכם מרוויח כסף ומצליחה! 🎉 
    חברות ענקיות כמו אפל או קוקה-קולה עובדות בדיוק ככה, פשוט בקנה מידה של כל העולם.
    """)
    st.write("🎥 **צפו בסרטון קצר שמסביר איך מתחילים עסק:**")
    st.video("https://www.youtube.com/watch?v=F3QpgXFtDeo")
    ...
    """)
    st.write("🎥 **צפו בסרטון שמסביר בדיוק מה זה אומר להיות שותפים (מה זו מניה):**")
    st.video("https://www.youtube.com/watch?v=ZCFkWDdmXG8")
    ...
    **2️⃣ דמי כיס (דיבידנד):** כשהחברה מרוויחה המון, היא מחלקת לשותפים שלה חלק מהרווחים במזומן ישר לחשבון! 💌
    """)
    st.write("🎥 **צפו בסרטון שמסביר איך הבורסה עובדת מאחורי הקלעים:**")
    st.video("https://www.youtube.com/watch?v=WEDIj9JBTC8")

    st.markdown("---")
    st.subheader("🧠 החידון של שוקי הינשוף!")
    st.write("בואו נבדוק מה למדנו עד עכשיו (5 שאלות שמתחלפות אוטומטית בכל פעם שאתם מרעננים):")

    # מאגר השאלות המורחב (8 שאלות, מתוכן יוגרלו 5)
    ALL_QUESTIONS = [
        ...
        {"q": "האם בורסה היא כמו קסם שמתעשרים ממנו ביום אחד?", "options": ["כן, תמיד מרוויחים בתוך יום!", "לא, זה דורש המון סבלנות, למידה והשקעה לאורך שנים", "כן, מספיק ללחוץ על כפתור", "לא, אי אפשר להרוויח בבורסה בכלל"], "answer": "לא, זה דורש המון סבלנות, למידה והשקעה לאורך שנים", "ok": "🎉 בדיוק! השקעה נכונה בבורסה היא כמו לשתול עץ - לוקח לו זמן לצמוח, אבל בסוף קוטפים פירות!", "bad": "❌ הבורסה היא לא קסם. כדי להרוויח בה באמת, צריך להשקיע בחברות טובות ולחכות בסבלנות הרבה זמן."}
    ]

    rng = random.Random(refresh_count)
    shown_qs = rng.sample(ALL_QUESTIONS, k=5) # עכשיו מוגרלות 5 שאלות במקום 2!
