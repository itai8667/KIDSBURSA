...
.block-container { direction: rtl !important; max-width: 1100px; padding-top: 1.2rem; }

h1, h2, h3, h4, h5, h6 { direction: rtl !important; text-align: right !important; font-family: 'Heebo', sans-serif !important; }
.stMarkdown p, .stMarkdown li, .stCaption, label { direction: rtl !important; text-align: right !important; font-family: 'Heebo', sans-serif !important; }
...
/* בטלפון מסתירים את הסיידבר לגמרי כדי שלא יכסה את כל המסך */
...
def calc_stars(row, w_growth, w_dividend, w_risk):
...
st.markdown('<div class="mobile-controls"></div>', unsafe_allow_html=True)
...
d_w_growth = st.sidebar.slider(" ", 0, 10, 5, key="desktop_growth")
...
d_w_dividend = st.sidebar.slider("  ", 0, 10, 5, key="desktop_div")
...
d_w_risk = st.sidebar.slider("   ", 1, 10, 5, key="desktop_risk")

# מובייל: בקרות בתוך הדף
st.markdown("<div class='mobile-card'><h3 style='margin-top:0;'>🕹️ איזה משקיע אתה?</h3><p style='color:#6b7280; font-size:13px;'>בטלפון שמנו את ההגדרות כאן בתוך המסך כדי שהסיידבר לא יכסה הכל.</p></div>", unsafe_allow_html=True)
m_w_growth = st.slider("🚀 כמה חשובה לי צמיחה מהירה?", 0, 10, 5, key="mobile_growth")
m_w_dividend = st.slider("💰 כמה חשוב לי לקבל דמי כיס?", 0, 10, 5, key="mobile_div")
m_w_risk = st.slider("🎢 כמה סיכון אני מוכן לקחת?", 1, 10, 5, key="mobile_risk")

# ברירת מחדל: משתמשים בערכי המובייל, כי בדסקטופ הם כמעט זהים ואם צריך המשתמש יראה את הסיידבר.
w_growth = m_w_growth
w_dividend = m_w_dividend
w_risk = m_w_risk
...
with tab1:
    ...
    for _, row in df.iterrows():
        stars = calc_stars(row, w_growth, w_dividend, w_risk)
    ...
    with col2:
        ...
        st.caption(f"התאמה אישית אליך: {calc_stars(sel, w_growth, w_dividend, w_risk)}")
...
with tab2:
    ...
    def get_infinite_questions_pool():
        pool = [
            ...
            {"q": "מה בדרך כלל יותר בטוח?", "options": ["לקנות מניה של חברה אחת בלבד", "לקנות קרן סל שכוללת המון חברות"], "answer": "לקנות קרן סל שכוללת המון חברות", "ok": "🎉 נכון! פיזור בין הרבה חברות = פחות סיכון!", "bad": "❌ כשקונים הרבה חברות יחד הסיכון יורד כי אנחנו לא תלויים רק בחברה אחת."},
