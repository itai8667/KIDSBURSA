...
.settings-box h4 { color: #4f46e5; margin-top: 0 !important; margin-bottom: 8px !important; }
...
FULL_KIDS_COMPANIES = {
    ...
    "MAT":  {"name": "מאטל 🏎️",      "desc": "יצרנית הצעצועים הענקית.",                "products": "ברבי, מכוניות הוט-ווילס (Hot Wheels) ומשחק הקלפים אונו.", "fun_fact": "בובת הברבי הראשונה בעולם עלתה רק 3 דולר!", "image": "https://upload.wikimedia.org/wikipedia/commons/thumb/2/25/Mattel_logo.svg/200px-Mattel_logo.svg.png", "risk": 7}
}
...
def fetch_kids_data(tickers):
    ...
    for ticker in tickers:
        ...
        try:
            ...
            rows.append({"ticker": ticker, "name": d["name"], "desc": d["desc"],
...
with tab2:
    ...
    # שלב 1 — מה זו חברה
    st.markdown("""
    <div style='background:#eff6ff;padding:16px;border-radius:14px;margin-bottom:8px;'>
    <h3 style='color:#1d4ed8;margin-top:0;'>🍋 שלב 1 — מה זו בכלל חברה?</h3>
    <p>תחשבו שפתחתם בקיץ <b>דוכן לימונדה</b> שכונתי. אם הרבה אנשים קונים מכם — הדוכן מרוויח כסף ומצליח! 🎉
    חברות ענקיות כמו אפל עובדות בדיוק ככה, רק בגדול.</p>
    </div>
    """, unsafe_allow_html=True)
    # סרטון עברי: איך עובד המסחר בבורסה — אליה מסביר דברים בעזרת צעצועים
    st.video("https://www.youtube.com/watch?v=TG7HL5PsMGw")

    st.markdown("<br>", unsafe_allow_html=True)

    # שלב 2 — מה זו מניה
    st.markdown("""
    <div style='background:#f0fdf4;padding:16px;border-radius:14px;margin-bottom:8px;'>
    <h3 style='color:#166534;margin-top:0;'>🧩 שלב 2 — מה זו בעצם מניה?</h3>
    <p>רוצים לפתוח <b>עוד 10 דוכנים</b> אבל אין כסף? אתם אומרים לחברים: "תנו לי קצת כסף, ובתמורה אתן לכם חתיכה מהדוכן שלי!"
    כל חתיכה כזאת נקראת <b>מניה</b>.</p>
    </div>
    """, unsafe_allow_html=True)
    # סרטון עברי: מה זו מניה — הסבר ברור
    st.video("https://www.youtube.com/watch?v=UQVzmQJ0OX0")

    st.markdown("<br>", unsafe_allow_html=True)

    # שלב 3 — איך מרוויחים
    st.markdown("""
    <div style='background:#fffbeb;padding:16px;border-radius:14px;margin-bottom:8px;'>
    <h3 style='color:#92400e;margin-top:0;'>💰 שלב 3 — איך מרוויחים מזה כסף?</h3>
    <p><b>1️⃣ צמיחה:</b> אם החברה מצליחה, כולם רוצים את ה"חתיכות" שלה. המחיר עולה ותוכלו למכור ברווח! 🤑<br>
    <b>2️⃣ דיבידנד:</b> כשהחברה מרוויחה המון, היא מחלקת לשותפים מזומן ישר לחשבון! 💌</p>
    </div>
    """, unsafe_allow_html=True)
    # סרטון עברי: מה זה דיבידנד — כאן חדשות
    st.video("https://www.youtube.com/watch?v=v_wIwfRQ7q8")

    st.markdown("<br>", unsafe_allow_html=True)

    # שלב 4 — השקעות לילדים
    st.markdown("""
    <div style='background:#fdf4ff;padding:16px;border-radius:14px;margin-bottom:8px;'>
    <h3 style='color:#7e22ce;margin-top:0;'>🌱 שלב 4 — שיעור השקעות לילדים</h3>
    <p>ילד שמשקיע מוקדם — מרוויח יותר! צפו בשיחה אמיתית בין אבא לבתו על השקעות בשוק ההון.</p>
    </div>
    """, unsafe_allow_html=True)
    # סרטון עברי: שיעור השקעות לילדים ומתחילים
    st.video("https://www.youtube.com/watch?v=q3rOeUGnxxA")
    ...
    def get_infinite_questions_pool():
        pool = [
            ...
            {"q": "מה בדרך כלל יותר בטוח?",
             ...
             "bad": "❌ כשקונים הרבה חברות יחד הסיכון יורד כי אנחנו לא תלויים רק בחברה אחת."},
        ...
        for _ in range(25):
            ...
            random.shuffle(opts)
            pool.append({"q": f"קניתם מניה ב-{buy} ₪, וכעבור שנתיים מכרתם אותה ב-{sell} ₪. כמה כסף הרווחתם בסך הכל?",
                         "options": opts, "answer": f"{profit} ₪",
            ...
            random.shuffle(opts)
            pool.append({"q": f"השקעתם {invest} ₪ בחברה מעולה שמחלקת דיבידנד של {div_p}%. כמה דמי כיס תקבלו?",
                         "options": opts, "answer": f"{div_money} ₪",
            ...
            random.shuffle(opts)
            pool.append({"q": f"קניתם מניה ב-{buy_2} ₪. לצערינו החברה הפסידה והמניה ירדה ל-{sell_2} ₪ ומכרתם אותה. כמה כסף הפסדתם?",
                         "options": opts, "answer": f"{loss} ₪",
    ...
    for i, item in enumerate(st.session_state.quiz_questions):
        ...
        choice = st.radio("בחר תשובה:", item["options"], index=None,
